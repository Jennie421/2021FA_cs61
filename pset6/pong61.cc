...
std::condition_variable congestion_cv;
std::mutex congestion_lock;
...
http_connection* http_connect(const struct addrinfo* ai) {
    ...
    std::unique_lock<std::mutex> cl(congestion_lock);
    while (congested) congestion_cv.wait(cl);
    cl.unlock();
...
bool move_done;
std::condition_variable move_done_cv;
std::mutex move_done_lock;

// a connection table of available connections
 std::deque<http_connection*> available_connections;
...
void pong_thread(int x, int y) {
    ...
    // if has available connections 
    while (!available_connections.empty()) {
        http_connection* candidate = available_connections[0];  // get current connection
        available_connections.pop_front();
        if (candidate->cstate_ == cstate_idle) {
            conn = candidate;   // use current connection is usaable
            break;
        }
    }

    // if no useable connection, build a new one
    if (conn == nullptr) {
    ...
    // phase 1 
    // close the dropped connection, and make a new connection at the same position.
    useconds_t wait_time = 10000;
    useconds_t max_wait_time = 128000000; 
    ...
    std::unique_lock<std::mutex> lock(move_done_lock);
    move_done = true;
    move_done_cv.notify_all();
    lock.unlock();
    ...
    if (result < 0) {
        ...
        printf("should sleep for %f ms\n", result);
        congested = true;
        std::unique_lock<std::mutex> cl(congestion_lock);
        usleep(result * 1024);
        congested = false;
        congestion_cv.notify_all();
        cl.unlock();
    }
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        move_done = false;*/
        std::unique_lock<std::mutex> lock(move_done_lock);
        while (!move_done) move_done_cv.wait(lock);
        move_done = false;
        lock.unlock();
